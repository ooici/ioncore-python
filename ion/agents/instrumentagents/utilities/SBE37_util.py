...
bin/mypython ion/agents/instrumentagents/utilities/SBE37_util.py [options] ipaddress port command
bin/mypython ion/agents/instrumentagents/utilities/SBE37_util.py -n 20 -f ion/agents/instrumentagents/utilities/SBE37_3464_config_short.txt 137.110.112.119 4001 reset 
bin/mypython ion/agents/instrumentagents/utilities/SBE37_util.py -n 20 -f ion/agents/instrumentagents/utilities/SBE37_3464_config.txt 137.110.112.119 4001 reset 
...
from ion.agents.instrumentagents.SBE37_driver import SBE37Prompt
...
def wakeup(sock,no_tries):
    ...
    while count < no_tries:
        ...
        if buffer.endswith(SBE37Prompt.PROMPT) \
            or buffer.endswith(SBE37Prompt.STOP_AUTOSAMPLE):
            break
        else:
            count += 1
...
def do_cmd(cmd,sock):
    ...
    sock.send(cmd+SBE37Prompt.NEWLINE)
    time.sleep(1)
    ...
    while True:
        try:
            ...
            if buffer.endswith(SBE37Prompt.PROMPT):
                break
            elif buffer.endswith(SBE37Prompt.STOP_AUTOSAMPLE):
                break
...
def reset(s, no_tries, config_file):
    ...
    if not buffer.endswith(SBE37Prompt.PROMPT) \
        and not buffer.endswith(SBE37Prompt.STOP_AUTOSAMPLE):
    ...
    if buffer.endswith(SBE37Prompt.STOP_AUTOSAMPLE):
