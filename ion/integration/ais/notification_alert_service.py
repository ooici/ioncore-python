...
class NotificationAlertService(ServiceProcess):
    ...
    def handle_update_event2(content):
            ...
            for key, row in rows.iteritems ( ) :
                ...
                if (rows[key]['subscription_type'] == SUBSCRIPTION_INFO_TYPE.SubscriptionType.EMAILANDDISPATCHER or rows[key]['subscription_type'] == SUBSCRIPTION_INFO_TYPE.SubscriptionType.EMAIL ) \
                    and rows[key]['email_alerts_filter'] == SUBSCRIPTION_INFO_TYPE.AlertsFilter.UPDATES :
                    # Send the message via our own SMTP server, but don't include the envelope header.
                    # Create the container (outer) email message.
                    FROM = 'OOI@ucsd.edu'
                    TO = rows[key]['user_email']
    ...
    def op_addSubscription(self, content, headers, msg):
        ...
        def handle_update_event(content):
        ...
        if ( content.message_parameters_reference.subscription_type == content.message_parameters_reference.SubscriptionType.EMAILANDDISPATCHER and (content.message_parameters_reference.email_alerts_filter == content.message_parameters_reference.AlertsFilter.UPDATES or content.message_parameters_reference.dispatcher_alerts_filter == content.message_parameters_reference.AlertsFilter.UPDATES) ) \
            or (content.message_parameters_reference.subscription_type == content.message_parameters_reference.SubscriptionType.EMAIL and content.message_parameters_reference.email_alerts_filter == content.message_parameters_reference.AlertsFilter.UPDATES) \
            or (content.message_parameters_reference.subscription_type == content.message_parameters_reference.SubscriptionType.DISPATCHER and content.message_parameters_reference.dispatcher_alerts_filter == content.message_parameters_reference.AlertsFilter.UPDATES):
