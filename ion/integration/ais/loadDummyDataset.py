...
class LoadDummyDataset(object):

    @defer.inlineCallbacks
    def loadDummyDataset(self, rc):
            
        
        # Create the dataset resource
        log.debug("LoadDummyDataset calling create_instance")
        dataset = yield rc.create_instance(dataset_type, ResourceName='Test AIS dataset',
                                           ResourceDescription='Dummy AIS dataset resource')
        
        # Attach the root group
        group = dataset.CreateObject(group_type)
        group.name = 'dummy data'
        ...
        dsID = dataset.ResourceIdentity

        defer.returnValue(dsID)
