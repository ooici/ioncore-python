...
This standalone script can probably be broken into a few different scripts. Currently 
the design can kindly be described as results-oriented.
"""
...
class CassandraPerformanceTester:
    ...
    def runQueryBenchMarks(self):
        ...
        @defer.inlineCallbacks
        def setup_indexes(f, rows_returned):
            b1 = f.read(1024)
            b2 = f.read(1024)
        ...
        @defer.inlineCallbacks
        def runQuery(q, pred_type):
            for i in range(50):
                t1 = time.time()
                rows = yield self.store.query(q)
                t2 = time.time()
                diff = t2 - t1
        ...
        @defer.inlineCallbacks
        def runQueries(terms):
            q1 = Query()
            q1.add_predicate_eq(self.indexes[0], terms[0])
            yield runQuery(q1, "1_EQ")
            
            q1.add_predicate_eq(self.indexes[1], terms[1])
            yield runQuery(q1, "2_EQ")
            q1.add_predicate_gt(self.indexes[2], "")
            yield runQuery(q1, "2_EQ_1_GT")
            q2 = Query()
            q2.add_predicate_eq(self.indexes[0], terms[0])
            q2.add_predicate_gt(self.indexes[2], "")
            yield runQuery(q2, "1_EQ_1_GT")
            
        
        self.query_blobs = dict(**self.blobs)
        f = open("/dev/urandom")
        t1 = time.time()
        terms1 = yield setup_indexes(f,1)
        terms10 = yield setup_indexes(f,10)
        terms100 = yield setup_indexes(f,100)
        terms1000 = yield setup_indexes(f,1000)
        ...
        yield runQueries(terms1)
        yield runQueries(terms10)
        yield runQueries(terms100)
        yield runQueries(terms1000)
        yield self.tearDown()
