...
def read_msg_from_dap_files(filename):
    ...
    @param filename A filename that has the same filestem of the three
    DAP files.
    """
...
def write_dap_files_from_msg(filename,msg):
    ...
    @param filename The filestem of the three files
    @param msg The ion message that has the DAP content
    """
    filestem, ext = os.path.splitext(filename)
...
def write_netcdf_from_dataset(dataset, filename):
    ...
    @param dataset dataset is the pydap object, which must contain
...
def demo_dataset():
    ...
    @see http://pydap.org/developer.html#the-dap-data-model
    """
