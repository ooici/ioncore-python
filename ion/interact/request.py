...
from ion.interact.conversation import ConversationType, Conversation, ConversationRole
from ion.util.state_object import FSMFactory, StateObject, BasicStates

class RequestInitiatorFSMFactory(FSMFactory):
    """
    A FSM factory for FSMs with conversation type state model for request.
    This is for the initiator role.
    """
    ...
    def create_fsm(self, target, memory=None):
        ...
        fsm.add_transition(self.E_DONE, self.S_AGREED, actionfct, self.S_COMPLETED)
...
class RequestParticipantFSMFactory(FSMFactory):
    """
    A FSM factory for FSMs with conversation type state model for request.
    This is for the participant role.
    """
...
class Request_Initiator(ConversationRole):
    factory = RequestInitiatorFSMFactory()

class Request_Participant(ConversationRole):
    factory = RequestParticipantFSMFactory()
